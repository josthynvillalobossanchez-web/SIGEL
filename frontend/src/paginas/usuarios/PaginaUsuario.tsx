...
type RolesMarcados = Record<string, Vigencia>;
...
function FormularioDeUsuario({ cuenta }: { cuenta?: DetalleDeCuenta }) {
    ...
    try {
      if (!cuenta) {
        ...
        const resultado = await editarCuenta(cuenta.id, {
          ...
          roles: cambioDeRoles ? pedidos() : undefined,
        });
  ...
  // Roles que se muestran: los activos, y los que la cuenta ya tiene (aunque
  // esten inactivos, para que se vean y se puedan quitar).
                        ...
                        <div className="opciones-en-linea">
                          <label className="opcion-chica">
                            <input
                              ...
                              onChange={() => setMarcados({ ...marcados, [rol.id]: { ...vigencia, conFecha: false } })}
                            />
                            Permanente
                          </label>
                          <label className="opcion-chica">
                            <input
                              ...
                              name={`vig-${rol.id}`}
                              checked={vigencia.conFecha}
                              onChange={() => setMarcados({ ...marcados, [rol.id]: { ...vigencia, conFecha: true } })}
                            />
                            Hasta el
                          </label>
                          {vigencia.conFecha && (
                            <CampoFechaDeVencimiento
                              ...
                              alCambiar={(fecha) => setMarcados({ ...marcados, [rol.id]: { ...vigencia, fecha } })}
